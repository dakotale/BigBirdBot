...
namespace DiscordBot.Modules
{
    public class JsonCommands : ModuleBase<SocketCommandContext>
    {
        ...
        [Command("kanye")]
        [Discord.Commands.Summary("Get a random tweet from Kanye's Twitter.")]
        public async Task HandleKanyeCommand()
        {
            try
            {
                audit.InsertAudit("kanye", Context.User.Username, Constants.Constants.discordBotConnStr);

                string apiUrl = "https://api.kanye.rest/?format=json";
                ...
                using (StreamReader reader = new StreamReader(stream))
                {
                    ...
                    List<Kanye> kanyes = kanye.GetKanye(connStr, results);
                    foreach (var o in kanyes)
                    {
                        var embed = new EmbedBuilder
                        {
                            Title = "BigBirdBot - Kanye Tweet",
                            ImageUrl = "https://i1.wp.com/culturalhistoryoftheinternet.com/wp-content/uploads/2020/11/cover2-1.jpg?resize=810%2C580&ssl=1",
                            Color = Color.Gold,
                            Description = o.KanyeText,
                        };
        ...
        [Command("insult")]
        [Discord.Commands.Summary("Have the bot insult you, if that's your thing.")]
        public async Task HandleInsultCommand()
        {
            try
            {
                audit.InsertAudit("insult", Context.User.Username, Constants.Constants.discordBotConnStr);
                ...
                using (StreamReader reader = new StreamReader(stream))
                {
                    ...
                    List<Insult> insults = insult.GetInsult(connStr, results);
                    foreach (var o in insults)
                    {
                        var embed = new EmbedBuilder
                        {
                            Title = "BigBirdBot - Insult",
                            Color = Color.Orange,
                            Description = o.InsultText,
                            ImageUrl = "https://st.depositphotos.com/2159851/3422/v/600/depositphotos_34227023-stock-illustration-angry-man-ranting-and-insulting.jpg"
                        };
