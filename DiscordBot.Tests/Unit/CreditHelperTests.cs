...
public class CreditHelperTests
{
    ...
    [Fact]
    public void EvaluatePokerHand_RoyalFlush()
    {
        var hand = new List<string> { "A|♠", "K|♠", "Q|♠", "J|♠", "10|♠" };
        Assert.Equal(CreditHelper.PokerHand.RoyalFlush, CreditHelper.EvaluatePokerHand(hand));
    }

    [Fact]
    public void EvaluatePokerHand_StraightFlush()
    {
        var hand = new List<string> { "9|♠", "8|♠", "7|♠", "6|♠", "5|♠" };
        Assert.Equal(CreditHelper.PokerHand.StraightFlush, CreditHelper.EvaluatePokerHand(hand));
    }

    [Fact]
    public void EvaluatePokerHand_AceLowStraightFlush()
    {
        var hand = new List<string> { "A|♠", "2|♠", "3|♠", "4|♠", "5|♠" };
        Assert.Equal(CreditHelper.PokerHand.StraightFlush, CreditHelper.EvaluatePokerHand(hand));
    }

    [Fact]
    public void EvaluatePokerHand_FourOfAKind()
    {
        var hand = new List<string> { "A|♠", "A|♥", "A|♦", "A|♣", "K|♠" };
        Assert.Equal(CreditHelper.PokerHand.FourOfAKind, CreditHelper.EvaluatePokerHand(hand));
    }

    [Fact]
    public void EvaluatePokerHand_FullHouse()
    {
        var hand = new List<string> { "A|♠", "A|♥", "A|♦", "K|♣", "K|♠" };
        Assert.Equal(CreditHelper.PokerHand.FullHouse, CreditHelper.EvaluatePokerHand(hand));
    }

    [Fact]
    public void EvaluatePokerHand_Flush()
    {
        var hand = new List<string> { "A|♠", "K|♠", "Q|♠", "J|♠", "9|♠" }; // not sequential
        Assert.Equal(CreditHelper.PokerHand.Flush, CreditHelper.EvaluatePokerHand(hand));
    }

    [Fact]
    public void EvaluatePokerHand_Straight()
    {
        var hand = new List<string> { "A|♠", "K|♥", "Q|♦", "J|♣", "10|♠" }; // mixed suits
        Assert.Equal(CreditHelper.PokerHand.Straight, CreditHelper.EvaluatePokerHand(hand));
    }

    [Fact]
    public void EvaluatePokerHand_AceLowStraight()
    {
        var hand = new List<string> { "A|♠", "2|♥", "3|♦", "4|♣", "5|♠" };
        Assert.Equal(CreditHelper.PokerHand.Straight, CreditHelper.EvaluatePokerHand(hand));
    }
    ...
    [Fact]
    public void EvaluatePokerHand_ThreeOfAKind()
    {
        var hand = new List<string> { "A|♠", "A|♥", "A|♦", "K|♣", "Q|♠" };
        Assert.Equal(CreditHelper.PokerHand.ThreeOfAKind, CreditHelper.EvaluatePokerHand(hand));
    }

    [Fact]
    public void EvaluatePokerHand_TwoPair()
    {
        var hand = new List<string> { "A|♠", "A|♥", "K|♦", "K|♣", "Q|♠" };
        Assert.Equal(CreditHelper.PokerHand.TwoPair, CreditHelper.EvaluatePokerHand(hand));
    }

    [Fact]
    public void EvaluatePokerHand_JacksOrBetter_WithJacks()
    {
        var hand = new List<string> { "J|♠", "J|♥", "A|♦", "K|♣", "Q|♠" };
        Assert.Equal(CreditHelper.PokerHand.JacksOrBetter, CreditHelper.EvaluatePokerHand(hand));
    }

    [Fact]
    public void EvaluatePokerHand_JacksOrBetter_WithAces()
    {
        var hand = new List<string> { "A|♠", "A|♥", "9|♦", "7|♣", "5|♠" };
        Assert.Equal(CreditHelper.PokerHand.JacksOrBetter, CreditHelper.EvaluatePokerHand(hand));
    }

    [Fact]
    public void EvaluatePokerHand_HighCard_PairOfTens_IsHighCard()
    {
        // Pair of 10s is below Jacks threshold — no win
        var hand = new List<string> { "10|♠", "10|♥", "A|♦", "K|♣", "Q|♠" };
        Assert.Equal(CreditHelper.PokerHand.HighCard, CreditHelper.EvaluatePokerHand(hand));
    }

    [Fact]
    public void EvaluatePokerHand_HighCard()
    {
        var hand = new List<string> { "2|♠", "4|♥", "6|♦", "8|♣", "A|♠" };
        Assert.Equal(CreditHelper.PokerHand.HighCard, CreditHelper.EvaluatePokerHand(hand));
    }
    ...
    [Theory]
    [InlineData(CreditHelper.PokerHand.RoyalFlush,    1_000, 800_000)]
    [InlineData(CreditHelper.PokerHand.StraightFlush, 1_000,  50_000)]
    [InlineData(CreditHelper.PokerHand.FourOfAKind,   1_000,  25_000)]
    [InlineData(CreditHelper.PokerHand.FullHouse,     1_000,   9_000)]
    [InlineData(CreditHelper.PokerHand.Flush,         1_000,   6_000)]
    [InlineData(CreditHelper.PokerHand.Straight,      1_000,   4_000)]
    [InlineData(CreditHelper.PokerHand.ThreeOfAKind,  1_000,   3_000)]
    [InlineData(CreditHelper.PokerHand.TwoPair,       1_000,   2_000)]
    [InlineData(CreditHelper.PokerHand.JacksOrBetter, 1_000,   1_000)]
    [InlineData(CreditHelper.PokerHand.HighCard,      1_000,       0)]
    public void PokerPayout_ReturnsCorrectMultiple(CreditHelper.PokerHand hand, decimal bet, decimal expected)
    {
        Assert.Equal(expected, CreditHelper.PokerPayout(hand, bet));
    }
