    ...
    public class CommandHandlingService
    {
        ...
        public async Task CommandExecutedAsync(Optional<CommandInfo> command, ICommandContext context, IResult result)
        {
            ...
            if (result.ErrorReason.Contains("The input text has too few parameters"))
            {
                ...
                EmbedBuilder embed = embedHelper.BuildMessageEmbed("Error", "An additional parameter is required in order to run this command.", Constants.Constants.errorImageUrl, "", Color.Red, "");
                await context.Channel.SendMessageAsync(embed: embed.Build());
            }
            else if (result.ErrorReason.Contains("Failed to parse"))
            {
                EmbedHelper embedHelper = new EmbedHelper();
                EmbedBuilder embed = embedHelper.BuildMessageEmbed("Error", "Please enter a whole number for this command.", Constants.Constants.errorImageUrl, "", Color.Red, "");
                await context.Channel.SendMessageAsync(embed: embed.Build());
            }
            else if (result.ErrorReason.Contains("Argument cannot be blank"))
            {
                EmbedHelper embedHelper = new EmbedHelper();
                EmbedBuilder embed = embedHelper.BuildMessageEmbed("Error", "The parameter(s) entered are not valid, please run the command with the correct parameters.", Constants.Constants.errorImageUrl, "", Color.Red, "");
                await context.Channel.SendMessageAsync(embed: embed.Build());
            }
            else if (result.ErrorReason.Contains("The input text has too many parameters") && command.Value.Name.Equals("Play"))
            {
                EmbedHelper embedHelper = new EmbedHelper();
                EmbedBuilder embed = embedHelper.BuildMessageEmbed("Error", "Your text search must be contained in quotes.\nExample: -play \"Video Search\"", Constants.Constants.errorImageUrl, "", Color.Red, "");
                await context.Channel.SendMessageAsync(embed: embed.Build());
            }
            else
            {
                EmbedHelper embedHelper = new EmbedHelper();
                EmbedBuilder embed = embedHelper.BuildMessageEmbed("Error", result.ErrorReason, Constants.Constants.errorImageUrl, "", Color.Red, "");
                await context.Channel.SendMessageAsync(embed: embed.Build());
            }
        }
    }
